...
{
    public partial class edit_qtd : Form
    {
        public edit_qtd(int preco, string code,int qtd)
        {
            InitializeComponent();
            Preco = preco;
            Barcode = code;
        ...
        private void btn_AddCarinho_Click(object sender, EventArgs e)
        {
            try
            {
                if (txt_Quant.Text != "")
                {
                    ...
                    cmd.CommandText = "UPDATE vendidos set Qtd = '" + txt_Quant.Text + "' where barcode = '" + Barcode + "'";
                    cmd.ExecuteNonQuery();
                    cmd.CommandText = "UPDATE vendidos set Total = '" + (int.Parse(txt_Quant.Text) * Preco) + "' where barcode = '" + Barcode + "'";
                    cmd.ExecuteNonQuery();
                    cmd.CommandText = "UPDATE save_vendidos set Qtd = '" + txt_Quant.Text + "' where barcode = '" + Barcode + "'";
                    cmd.ExecuteNonQuery();
                    cmd.CommandText = "UPDATE save_vendidos set Total = '" + (int.Parse(txt_Quant.Text) * Preco) + "' where barcode = '" + Barcode + "'";
                    cmd.ExecuteNonQuery();
